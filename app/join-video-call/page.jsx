...
import DeviceCheck from '../video-call/components/DeviceCheck';

import { useVideoRoom } from '../video-call/context/VideoRoomContext';

const COMPANY = process.env.NEXT_PUBLIC_COMPANY_NAME || 'recreatorsdesign';
const LOGO = process.env.NEXT_PUBLIC_COMPANY_LOGO_URL || '/assets/images/logos/RC-logo.png';

/* ---------- fallback shown while Suspense is resolving ---------- */
...
/* ---------- the real component (uses useSearchParams) ---------- */
function JoinVideoCallContent() {
  const { meetingId: routeMeetingId } = useParams();
  const [searchParams] = useSearchParams();
  const navigate = useRouter();
  const { enterRoom } = useVideoRoom();

  const [step, setStep] = useState('loading'); // loading | form | device | joining | blocked
  ...
  const resolveAndLoad = async () => {
    ...
    try {
      ...
      navigate.replace(`/video-call/${data.meeting.meetingId}`);
      setStep('form');
    } catch (err) {
  ...
  const handleJoinWithDevice = async () => {
    ...
    try {
      ...
      navigate.replace(`/video-call/${meeting.meetingId}/room`);
    } catch (err) {
    ...
    <>
      ...
      {step === 'blocked' && (
        <div className="!flex !flex-col !items-center !gap-4 !py-12 !text-center">
          ...
          <button
            onClick={() => navigate.push('/join-video-call')}
